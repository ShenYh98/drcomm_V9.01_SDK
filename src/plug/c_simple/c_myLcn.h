...
class CMyLcn_C: public CLcnIF
{
	...
	INT32			SendTaskNo;													// 发送帧顺序
	INT32			DevNum;														// 装置数量
	INT32			CurrDevNo;													// 当前查询装置序号
	INT32			*pDevAddr;													// 装置地址组

	std::clock_t 	start_time;													//定时器

	INT32			SendIDTaskNo;												// 自动分配id任务发送帧顺序
	UINT8			IDtotal;													// 自动分配id总数
	bool			IsRecv;														// 是否回复了报文
	std::vector<DevFeature> v_devFeature;										// 设备特征
public:
...
public:
	...
	*/
	...
	INT32 AssembleAssignIDTask(UINT8 *buf, const INT32 bufferSize, INT32& size);		// 组装分配id任务
	INT32 AssembleDestroyID(UINT8 *buf, const INT32 bufferSize);  		//删除id
	INT32 AssembleQueryID(UINT8 *buf, const INT32 bufferSize);  		//查询id
	INT32 AssembleDownloadID(UINT8 *buf, const INT32 bufferSize);   	//下设id
	...
	*/
	...
	INT32 ProcessGetSN(const UINT8* buf, const INT32 len);				// 获取SN号
};
